...
Outside edit mode, double-click or a visible control opens the same Widget entry in a large modal; Escape, the close control, or the mask returns to the canvas. The Host selects `window.dshWidget.displayMode` as `compact` or `expanded`. This presentation state is independent from the three manifest sizes and does not change the Widget id, state document, network permissions, sandbox policy, or entry path.
...
**Add a second expanded entry to the manifest.** Rejected because compact and expanded presentations share behavior, permissions, and state. One Host-selected display value lets ordinary responsive HTML reveal more detail without duplicating project lifecycle or security configuration.
...
The provider deliberately copies only the manifest and one self-contained HTML entry. Compact and expanded presentations remount that entry and therefore must restore interactive values from Host-owned state rather than frame memory. The authoring flow reuses the ordinary Workspace, Session, composer, and details column instead of adding a separate editor. Preview-open preference remains browser-local presentation state, while Widget data and canvas placement are Host-owned files rather than Session-log data. The platform has no asset directory, credential bridge, interval scheduler, approval UI, updates, multi-instance identity, disable state, or separate full-application entry. Those additions require separate decisions because they broaden durable state or runtime authority.
