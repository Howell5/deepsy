...
退出编辑模式后，双击或可见控件会在大尺寸模态框中打开同一个 Widget 入口；Escape、关闭控件或遮罩会返回画布。宿主把 `window.dshWidget.displayMode` 设为 `compact` 或 `expanded`。这种呈现状态独立于 manifest 的三种尺寸，不改变 Widget id、状态文档、网络权限、沙箱策略或入口路径。
...
卡片通过 `iframe sandbox="allow-scripts"` 按照选中的语义尺寸或完整模式渲染宿主校验后的 HTML。父页面注入呈现模式和固定画布规则，在不缩放内容的前提下禁止滚动。布局观察器向父页面报告内容固有溢出；父页面会用布局无效错误替换发生溢出的文档，生成页面必须适配它声明的每一种尺寸。父页面还会注入内容安全策略，拒绝直接连接、导航权限、Node 集成和文件系统访问。嵌入脚本通过 `postMessage` 调用 `window.dshWidget.fetch(url)`、`state.get()` 和 `state.set(nextState)`；父页面只接受来自该 frame 的请求，自行提供调用方 Widget id，再把请求转发到带类型的宿主 RPC 域。
...
**在 manifest 中增加第二个完整模式入口。** 否决，因为紧凑与完整呈现共享行为、权限和状态。一个由宿主选择的呈现值就能让普通响应式 HTML 显示更多细节，无需复制项目生命周期或安全配置。

## 后果

桌面端无需系统 Node.js 或项目专属服务器即可运行实用的本地 Widget，联网 Widget 也只有一个可审计的宿主中介点。同一组应用 slot 可以承载后续一级桌面界面，而不必把它们做成 Session 视图。

提供方有意只复制 manifest 和一个自包含 HTML 入口。紧凑与完整呈现会重新挂载该入口，因此交互值必须从宿主持有的状态恢复，不能依赖 frame 内存。创作流程复用普通 Workspace、Session、输入框和详情栏，不新增独立编辑器。预览打开偏好仍是浏览器本地展示状态，而 Widget 数据和画布位置是宿主拥有的文件，不属于 Session 日志数据。平台没有资源目录、凭证桥接、间隔调度器、权限审批 UI、更新、多实例身份、停用状态或独立的完整应用入口。这些扩展会扩大持久状态或运行时权限，因此需要独立决策。
