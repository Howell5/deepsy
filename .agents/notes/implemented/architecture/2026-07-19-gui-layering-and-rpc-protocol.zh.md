...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - Electron 应用复用同一套 web client 包。首个启动版本经由操作系统分配的回环端口使用 Web 载体；生产硬化目标仍为 IPC fetch 载体，详见[桌面端启动说明](2026-08-14-electron-desktop-bootstrap.zh.md)。
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web）+ `dsh-desktop`（apps/desktop） | 命令分发、Vite 应用与复用同一 Web 组合的 Electron 壳 | 各应用使用动态 import，因此不会互相加载；dist 定位等 workspace 知识留在 app |
