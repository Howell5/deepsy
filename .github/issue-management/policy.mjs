...
const targetRepository = repositoryCoordinates(process.env.GITHUB_REPOSITORY)
...
/**
 * Resolve the repository receiving the current event.
 * @param {string | undefined} value GitHub `owner/repository` slug.
 * @returns {{owner: string, repository: string, slug: string}} Parsed coordinates.
 */
export function repositoryCoordinates(value) {
  const slug = value ?? `${config.organization}/${config.repository}`
  const parts = slug.split('/')
  if (parts.length !== 2 || parts.some(part => part.length === 0)) {
    throw new Error(`GITHUB_REPOSITORY 格式无效：${slug}`)
  }
  return { owner: parts[0], repository: parts[1], slug }
}
...
async function issueSnapshot(number, status = undefined) {
  const issue = await api(`/repos/${targetRepository.slug}/issues/${number}`)
  if (issue.pull_request) return null
  const values = await api(
    `/repos/${targetRepository.slug}/issues/${number}/issue-field-values?per_page=100`,
  )
...
async function upsertAudit(number, errors) {
  const comments = await api(
    `/repos/${targetRepository.slug}/issues/${number}/comments?per_page=100`,
  )
  ...
  if (errors.length === 0) {
    if (existing) {
      await api(`/repos/${targetRepository.slug}/issues/comments/${existing.id}`, {
        method: 'DELETE',
      })
    }
    return
  ...
  if (existing) {
    ...
    await api(`/repos/${targetRepository.slug}/issues/${number}/comments`, {
      method: 'POST',
      body: JSON.stringify({ body }),
      headers: { 'Content-Type': 'application/json' },
    })
...
async function resolvingReferencesSnapshot(number, pull) {
  const references = parseReferences({
    body: pull.body ?? '',
    repository: targetRepository.slug,
  })
...
async function pullRequestSnapshot(number) {
  const [pull, reviewRequests, reviews] = await Promise.all([
    api(`/repos/${targetRepository.slug}/pulls/${number}`),
    api(`/repos/${targetRepository.slug}/pulls/${number}/requested_reviewers`),
    api(`/repos/${targetRepository.slug}/pulls/${number}/reviews?per_page=100`),
  ])
...
async function lifecyclePullRequestSnapshot(number) {
  const pull = await api(`/repos/${targetRepository.slug}/pulls/${number}`)
