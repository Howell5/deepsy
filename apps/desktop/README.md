...
deepsy is the Electron shell for the shipped DeepSeek Harness Web client. The renderer runs with Chromium sandboxing, no Node integration, context isolation, and denied permission requests. The Harness profile runs in an Electron Utility Process and listens on an operating-system-assigned loopback port.
...
This builds the host libraries, browser bundles, and desktop entry before opening Electron. The resulting application does not use a system Node.js runtime; Electron supplies the runtime used by the Utility Process.
