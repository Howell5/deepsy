...
| `ctx.widgets` | `seam` | [`widgets`](../packages/widget/widgets) | [`widgets-local`](../packages/widget/widgets-local) | [`widgets-local`](../packages/widget/widgets-local) | - | Service Definition 拥有严格的 manifest 与生命周期操作；本地提供方拥有受管理文件和网络策略，宿主网关提供浏览器可调用的 RPC 域。 |
