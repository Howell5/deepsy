...
The desktop workspace uses the complete center area as a logical grid. Edit mode places an interaction layer over each iframe so the whole card can be dragged or moved with arrow keys; pointer targets snap to cells and avoid occupied spans. The Host persists explicit placements, while a narrow window may temporarily clamp and reflow them without overwriting the saved desktop layout. Outside edit mode, double-click or an explicit control opens the same entry in a modal and Escape closes it. `window.dshWidget.displayMode` distinguishes `compact` from `expanded` presentation without changing manifest size, Widget identity, state, permissions, or the sandbox policy. Agent-authored scripts call `window.dshWidget.state.get()` and `state.set(nextState)` through the same source-checked `postMessage` bridge as network fetches; the frame cannot choose another Widget id or access the storage files.
