...
describe('Issue lifecycle workflow', () => {
  it('runs the lifecycle job on every PR/review event but gates token and board steps', () => {
    ...
    const gated = "${{ env.DSH_ISSUE_APP_CLIENT_ID != '' && env.DSH_ISSUE_APP_PRIVATE_KEY != '' && (github.event_name != 'pull_request_review' || github.event.review.state == 'changes_requested') }}"
...
describe('deepsy desktop release workflow', () => {
  it('can publish the current workspace version without creating a bump commit', () => {
    ...
    const decision = steps.find(step => step.name === 'Synchronize upstream source')
    const build = steps.find(step => step.name === 'Build desktop artifacts')
    const push = steps.find(step => step.name === 'Commit and push validated source')
