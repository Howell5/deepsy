...
import type { WidgetId, WidgetLayoutItem, WidgetState, WidgetView } from '@deepseek-ai/dsh-widgets/types'
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixtureWidget: WidgetView = {
    manifest: {
      schemaVersion: 2,
      id: brandString<WidgetId>('calculator'),
      name: 'Calculator',
      version: '1.0.0',
      runtime: 'static',
  ...
  const rpc: ClientConnectionRpc = {
    call(channel, endpoint, payload, signal) {
      ...
      switch (endpoint) {
        case 'widgets/list': return sessionOk({ widgets: [...fixtureWidgets] })
        case 'widgets/create': {
          const id = brandString<WidgetId>(`widget-${nextWidget++}`)
          const widget: WidgetView = {
            ...fixtureWidget,
            manifest: { ...fixtureWidget.manifest, id, name: 'New Widget', version: '0.1.0' },
            sourcePath: `${FIXTURE_HOME}/Widgets/${id}`,
            builtIn: false,
          }
          fixtureWidgets.push(widget)
          return sessionOk({ widget })
        }
        case 'widgets/read': {
          const widget = fixtureWidgets.find(candidate => candidate.manifest.id === args.id)
          return widget === undefined
            ? sessionErr({ code: 'gateway/internal', message: 'Widget not found', details: {} })
            : sessionOk({
              widget,
        ...
        case 'widgets/stateRead': return sessionOk({ state: fixtureWidgetState.get(args.id ?? '') ?? {} })
        case 'widgets/stateWrite':
          fixtureWidgetState.set(args.id ?? '', args.state ?? {})
          return sessionOk(undefined)
        case 'widgets/layoutRead': return sessionOk({ layout: fixtureWidgetLayout })
        case 'widgets/layoutWrite':
          fixtureWidgetLayout = args.layout ?? []
          return sessionOk(undefined)
